...
export default function StudyPage() {
  ...
  useEffect(() => { studiedIds.length > 0 && localStorage.setItem(STORAGE_KEY, JSON.stringify(studiedIds)) }, [studiedIds])
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        let attempts = 0
        while (studiedIds.includes(p.id) && attempts < 20) {
  ...
  if (isLoading && !painting) {
    return <div style={{ padding: '24px 16px', textAlign: 'center' }}>Загрузка...</div>
  }
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  background: 'white', borderRadius: '50%', width: '44px', height: '44px',
